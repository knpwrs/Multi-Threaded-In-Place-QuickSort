...
public static class SorterComparator {
     ...
     */
    public static <T> void quicksort(T[] input,Comparator<T> comparator) {
        final AtomicInteger count = new AtomicInteger(1);
        pool.execute(new QuicksortRunnable<T>(input,comparator, 0, input.length - 1, count));
        try {
            synchronized (count) {
                count.wait();
            }
     ...
     */
    private static class QuicksortRunnable<T> implements Runnable {
        /**
         ...
         */
        public QuicksortRunnable(T[] values, Comparator<T> comparator, int left, int right, AtomicInteger count) {
            this.values = values;
            this.left = left;
            this.right = right;
            this.count = count;
        ...
        private void quicksort(int pLeft, int pRight) {
            if (pLeft < pRight) {
                ...
                if (count.get() >= FALLBACK * N_THREADS) {
                    ...
                    count.getAndAdd(2);
                    pool.execute(new QuicksortRunnable<T>(values, comparator, pLeft, storeIndex - 1, count));
                    pool.execute(new QuicksortRunnable<T>(values, comparator, storeIndex + 1, pRight, count));
                }
            }
        }
        ...
        private int partition(int pLeft, int pRight) {
            ...
            for (int i = pLeft; i < pRight; i++) {
                if (this.comparator.compare(values[i], pivotValue) < 0) {
                    swap(i, storeIndex);
                    storeIndex++;
                }
            }
